...
var removeDuplicates = function(nums) {
    ...
    for (let j = 1; j < nums.length; j++) {
        if (nums[j] !== nums[i]) { // Found a new unique element
            ...
            nums[i] = nums[j]; // Overwrite nums at index i
        }
    }

    return i + 1; // The number of unique elements (k)
};
 ...
 * Time and Space Complexity of the Optimized Approach
    ...
    Space Complexity:
    No additional space is used other than a few variables (i and j), so the space complexity is O(1).
